...
{
	return [[self class] enumeratorAtPath: fpath];
}

+(id)					enumeratorWithPath: (NSString*)fpath cacheSize: (ItemCount)n
{
	return [[self class] enumeratorAtPath: fpath cacheSize: n];
}
#endif
...
-(id)	initWithFSRef: (FSRef)theFile cacheSize: (ItemCount)n desiredInfo: (FSCatalogInfoBitmap)desInfo
{
	self = [super init];
	if( self )
	{
		...
		if( (err = FSOpenIterator( &theFile, kFSIterateFlat, &iterator )) != noErr )
		{
			NSLog(@"UKDirectoryEnumerator::initWithFSRef: - MacOS Error ID= %d",err);
			[self autorelease];
			return nil;
		}
        
        NSString*	fpath = [NSString stringWithFSRef: &container]; // In case it's a link or an alias that got resolved.
...
-(id)		initWithPath: (NSString*)fpath cacheSize: (ItemCount)n
{
	...
	{
		self = [super init];
		NSLog(@"UKDirectoryEnumerator::initWithPath: Can't get an FSRef for the specified path.");
		[self autorelease];
		return nil;
	}
	
	self = [self initWithFSRef: container cacheSize: n desiredInfo: kFSCatInfoNone];
...
-(FSRef*)	nextObjectFSRef
{
	if( currIndex >= foundItems )
	{
		OSErr err = FSGetCatalogInfoBulk( iterator, cacheSize, &foundItems,
                                            ...
                                            cache, (FSSpec*) NULL,
											(HFSUniStr255*) NULL);	// TODO: We could use this param to get the names for a faster -nextObject implementation.
		if( err != noErr && err != errFSNoMoreItems )
		{
			NSLog(@"UKDirectoryEnumerator::nextObjectFSRef - MacOS Error ID= %d",err);
			return nil;
		}
		...
		if( foundItems == 0 )
        {
            if( err != errFSNoMoreItems )	// No need to show error if we're just at end of list.
                NSLog(@"UKDirectoryEnumerator::nextObjectFSRef - FSCatalogInfoBulk returned 0 items, but not errFSNoMoreItems.");
			return nil;
        }
	}
...
{
	return [NSString stringWithFSRef: [self nextObjectFSRef]];	// stringWithFSRef returns NIL when it gets a NIL FSRef.
}
...
-(id)   nextObject
{
    NSString*   fname = [NSString stringWithFSRef: [self nextObjectFSRef]];	// stringWithFSRef returns NIL when it gets a NIL FSRef.
    if( !fname )
        return nil;
...
{
	return !([self isInvisible] || ([[[NSString stringWithFSRef: &(cache[currIndex -1]) ] lastPathComponent] characterAtIndex: 0] == '.'));
}
...
NSDictionary*       UKDictionaryFromFSCatInfo( FSCatalogInfo* currInfo, FSCatalogInfoBitmap whichInfo )
{
	...
	if( UK_BTST(whichInfo, kFSCatInfoFinderInfo) )
	{
		...
		if( UK_BTST(fInfo->finderFlags, kIsAlias) )
			[dict setObject: NSFileTypeSymbolicLink forKey: NSFileType];
	...
	if( UK_BTST(whichInfo, kFSCatInfoPermissions) )
	{
		...
		[dict setObject: [NSNumber numberWithUnsignedShort: (pInfo->mode & 0x7FF)] forKey: NSFilePosixPermissions];
	}
